...
<?php if (@$_GET['action'] === 'hitung_ulang'): ?>
	...
	<?php
				for ($j=0; $j < $n_cluster; $j++) {
					$query_centroid = "SELECT AVG(NILAI_NORMALISASI) as 'NILAI_NORMALISASI', AVG(PRESTASI_NONAKADEMIK_NORMALISASI) as 'PRESTASI_NONAKADEMIK_NORMALISASI', AVG(PERILAKU_NORMALISASI) as 'PERILAKU_NORMALISASI', AVG(ABSENSI_NORMALISASI) as 'ABSENSI_NORMALISASI' FROM nilai_siswa_manhattan WHERE CLUSTER = 'CLUSTER-".($j+1)."'";
					...
					while ($row_centroid = $resultat_centroid->fetch()) {
						...
						$query_update = "UPDATE `centroid_manhattan` SET `NILAI`= $update_nilai,`PRESTASI_NONAKADEMIK`= $update_ext, `PERILAKU`= $update_perilaku,`ABSENSI`= $update_absensi WHERE `ID_CENTROID` = $j";
			...
			<?php
				...
				while ($row = $resultat->fetch()) {
					...
					for ($j=0; $j < $n_cluster; $j++) {
						...
						$nilai_ekstra = pow(abs($row['PRESTASI_NONAKADEMIK_NORMALISASI']-@$centroid[$j]['PRESTASI_NONAKADEMIK_NORMALISASI']),2);
