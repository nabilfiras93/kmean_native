...
<?php if (@$_GET['action'] === 'hitung_ulang'): ?>
	<?php
		$query_total_data = "SELECT COUNT(*) as 'SIZE' FROM nilai_siswa_minkowski";
	...
	?>
			...
			<?php
				...
				while ($row = $resultat->fetch()) {
					...
					for ($j=0; $j < $n_cluster; $j++) {
						...
						$nilai_cluster = pow(($nilai_normal + $nilai_ekstra + $nilai_perilaku + $nilai_absen),1);
		...
		<?php
					...
					$query_update = "UPDATE `nilai_siswa_minkowski`  SET `CLUSTER`= '$cluster' WHERE `ID_NILAI` = $id_nilai";
					$bdd->query($query_update) or die(print_r($bdd->errorInfo()));
				}
		?>
			</tbody>
	...
	<?php
			...
			if ($i <= 0) {
				for ($k=0; $k < $n_cluster; $k++) { 
					$query_old_iterasi = "SELECT `ID_NILAI`, `CLUSTER` FROM `nilai_siswa_minkowski` WHERE `CLUSTER` = 'CLUSTER-".($k+1)."'";
...
<?php else: ?>
	<div class="main-content-inner">
		...
		<div style="margin: 15px 45px;">
			<a href="?hal=normalisasi_minkowski&action=normalisasi_data">
			...
			<div id="Tinggi" class="w3-container city" style="display:none">
			  <h2>Kategori Siswa Tinggi</h2>
				  <table class="table table-bordered small datatablesku">
					...
					<tbody>
						<?php 
							$query = "SELECT * FROM nilai_siswa_minkowski WHERE CLUSTER='CLUSTER-1'";
							$resultat = $bdd->query($query) or die(print_r($bdd->errorInfo()));
							$bantu = 1;
			...
			<div id="Sedang" class="w3-container city" style="display:none">
			  	...
			  	<table class="table table-bordered small datatablesku">
					...
					<tbody>
						<?php 
							$query = "SELECT * FROM nilai_siswa_minkowski WHERE CLUSTER='CLUSTER-2'";
							$resultat = $bdd->query($query) or die(print_r($bdd->errorInfo()));
							$bantu = 1;
			...
			<div id="Rendah" class="w3-container city" style="display:none">
			  	...
			  	<table class="table table-bordered small datatablesku">
					...
					<tbody>
						<?php 
							$query = "SELECT * FROM nilai_siswa_minkowski WHERE CLUSTER='CLUSTER-3'";
							$resultat = $bdd->query($query) or die(print_r($bdd->errorInfo()));
							$bantu = 1;
