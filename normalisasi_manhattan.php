<?php if (@$_GET['action'] === 'normalisasi_data'): ?>
	<?php
		...
		while ($row = $resultat->fetch()) { 
			...
			$query_save = "UPDATE `nilai_siswa_manhattan` SET `NILAI_NORMALISASI`=ROUND('$hasil_normalisasi_nilai', 2),`PRESTASI_NONAKADEMIK_NORMALISASI`=ROUND('$hasil_normalisasi_extrakulikuler', 2),`PERILAKU_NORMALISASI`=ROUND('$hasil_normalisasi_prilaku', 2),`ABSENSI_NORMALISASI`=ROUND('$hasil_normalisasi_absensi', 2) WHERE `ID_NILAI`='$id'";
